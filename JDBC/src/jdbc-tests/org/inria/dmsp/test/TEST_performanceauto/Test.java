package org.inria.dmsp.test.TEST_performanceauto;
...
public class Test extends Tools implements ITest
{
	...
	protected void run4(PrintWriter out, String dbmsHost) throws Exception
	{
		...
		authent[0] = "1022"; authent[1]= "1018"; // Dupont / Medecin 
		...
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_TEST.EP_QUERY_NOAC_SELECT_USER_BETWEEN_NOM);
		...
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_UI.EP_QUERY_AC_SELECT_INFO_BY_FORM);
		...
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_TEST.EP_QUERY_AC_SELECT_INFO_BY_FORM_BETWEEN_EVENT);
		...
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_TEST.EP_QUERY_AC_SELECT_INFO_COMMENT_BY_FORM);
		rs = Tools_dmsp.Test_SELECT_BY_INT(1012, ps);
		lireResultSet(rs, out);
		...
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_TEST.EP_QUERY_AC_SELECT_INFO_COMMENT_BY_FORM_BETWEEN_EVENT);
		rs = Tools_dmsp.Test_SELECT_BY_INT_AND_INT_AND_INT(1012, 2970, 3192, ps);
		lireResultSet(rs, out);
		...
		Shutdown_DBMS();
		authent[0] = "0"; authent[1]= "1"; // Willemant / Patient (no right to GIR Med, access to fiche de liaison) 
		openConnection(dbmsHost, authent);
		...
		ps = ((org.inria.jdbc.Connection)db).prepareStatement(DMSP_QEP_IDs.EP_TEST.EP_QUERY_AC_SELECT_EVENTnop_INFO_COMMENT_BY_EVENT);
		rs = Tools_dmsp.Test_SELECT_BY_INT(1332, ps); 	
		lireResultSet(rs, out); 
