...
public class StoreFile 
{
	public static void storeFile(String fileToSendPath, User user ){
		...
		try {
			...
			// send file to RS
			// all the information will be sent encrypted
			if (status == Constants.OK) {
				String file = Base64.encode(encryptFile);
				String EncryptedFileName = new File(encryptFilePath).getName();
				byte[] EncryptedIV = AsymmetricEncryption.encryptBlockByBlock(iv, pubKey);
				ServerSaveFile.serverSaveFile(file, EncryptedFileName, Base64.encode(encrSkey), Base64.encode(EncryptedIV),user, "STORE");
			}
