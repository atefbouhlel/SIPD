...
public class TCellDaemon{
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			List<EncryptedUser> EncryptedUsers = new ArrayList<EncryptedUser>();

			if (!users.isEmpty()){
				PublicKey pubKey = Tools.stringToPublicKey(myInfo.getMyPubKey());
				
				for (User contact : users) {
					byte[] EncryptedUserGID = AsymmetricEncryption.encryptBlockByBlock( Integer.toString(contact.getUserGID()).getBytes(), pubKey);
					...
					String strEncryptedUserGID = Base64.encode(EncryptedUserGID);
					...
					EncryptedUser EncryptedUser = new EncryptedUser(strEncryptedUserGID, strEncrTCellIP, strEncrPort, strEncrPubKey);
			...
			SendUserInfo.sendUserInfo(user, EncryptedUsers);
